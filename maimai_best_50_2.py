...
class DrawBest(object):
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        self.qqId = qqId
        self.static = 'static2'
        self.maimai_dir = os.path.join(self.static, 'mai', 'pic')
        self.cover_dir = os.path.join(self.static, 'mai', 'cover')
    ...
    async def whiledraw(self, data: List[ChartInfo], type: bool) -> Image.Image:
        ...
        for num, info in enumerate(data):
            ...
            cover = Image.open(os.path.join(self.cover_dir, f'{info.song_id}.png')).resize((135, 135))
            version = Image.open(os.path.join(self.maimai_dir, f'{info.type.upper()}.png')).resize((55, 19))
            ...
            dxscore = sum(total_list.by_id(str(info.song_id)).charts[info.level_index]['notes']) * 3
            diff_sum_dx = info.dxScore / dxscore * 100
            dxtype, dxnum = dxScore(diff_sum_dx)
            if dxnum:
                self._im.alpha_composite(Image.open(os.path.join(self.maimai_dir, f'UI_GAM_Gauge_DXScoreIcon_0{dxnum}.png')), (x + 335, y + 102))
    ...
    async def draw(self):
        meiryo = os.path.join(self.static, 'meiryo.ttc')
        siyuan = os.path.join(self.static, 'SourceHanSansSC-Bold.otf')
        Torus_SemiBold = os.path.join(self.static, 'Torus SemiBold.otf')
        basic = Image.open(os.path.join(self.maimai_dir, 'b50_score_basic.png'))
        advanced = Image.open(os.path.join(self.maimai_dir, 'b50_score_advanced.png'))
        expert = Image.open(os.path.join(self.maimai_dir, 'b50_score_expert.png'))
        master = Image.open(os.path.join(self.maimai_dir, 'b50_score_master.png'))
        remaster = Image.open(os.path.join(self.maimai_dir, 'b50_score_remaster.png'))
        ...
        self.fsl = {'fs': 'FS', 'fsp': 'FSp', 'fsd': 'FSD', 'fsdp': 'FSDp', 'sync': 'SP'}

        # 作图
        self._im = Image.open(os.path.join(self.maimai_dir, 'b50_bg.png')).convert('RGBA')

        self._im.alpha_composite(logo, (5, 130))
        if self.plate:
            plate = Image.open(os.path.join(self.maimai_dir, f'{self.plate}.png')).resize((1420, 230))
        else:
            plate = Image.open(os.path.join(self.maimai_dir, 'UI_Plate_300501.png')).resize((1420, 230))
        ...
        for n, i in enumerate(Rating):
            self._im.alpha_composite(Image.open(os.path.join(self.maimai_dir, f'UI_NUM_Drating_{i}.png')).resize((28, 34)), (760 + 23 * n, 137))
        ...
        self._mr = DrawText(text_im, meiryo)
        self._sy = DrawText(text_im, siyuan)
        ...
        self._mr.draw(900, 2465, 35, f'Designed by Yuri-YuzuChaN & BlueDeer233 | Generated by Rakki BOT', (0, 50, 100, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate50_2(payload: Dict) -> Tuple[Optional[Image.Image], bool]:
